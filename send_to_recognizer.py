...
reg_list = {'recogn_cr': reg_list_cr, 'test': reg_list_mo}

#e-mail для рассылки
mail_addr = {'recogn_cr': ['fly220@mail.ru','drebezowa@mail.ru','chaus@firevolonter.ru'], 'test': ['chaus@firevolonter.ru']}
...
def make_table_for(conn,cursor,src_tab,critical,period,reg_list,whom):
    subs_tab = 'for_' + whom
    statements = (
        """
		...
		"""%(subs_tab),
        """
        INSERT INTO %(w)s (name,acq_date,acq_time,satellite,region,critical,rating,peat_id,peat_district,peat_class,peat_fire,ident,geog)
            SELECT
                %(s)s.name,
                %(s)s.acq_date,
                %(s)s.acq_time,
...
def drop_whom_table(conn,cursor, whom):
    ...
    try:
        ...
        log(logfile, 'Error evaluating points:$s'%e)
...
def send_to_dispatcher_job():
    ...
    for subscriber in subscribers:
        num_points = make_table_for(conn,cursor,year_tab,critical,period,reg_list,subscriber)
        if period == '24 hours':
